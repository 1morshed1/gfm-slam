...
(GPU-1) via NVML/pynvml, NOT on Jetson. When Jetson is revisited, add a tegrastats path.
...
Usage (target):
    python measure_energy.py --gpu 1 --hz 50 --run "<slam cmd>" --out results/<exp>.json
"""
import argparse, sys
...
def main():
    ...
    ap.add_argument("--gpu", type=int, default=1, help="physical GPU index (pin to 1)")
    ...
    ap.add_argument("--out")
    ap.parse_args()
    print("STUB — implement in Phase 2 (NVML on rig GPU-1). See docstring.", file=sys.stderr)
    sys.exit(1)
