#!/usr/bin/env bash
# Phase 0 bring-up — gated. Run on the rig (vm-130-131), GPU-1.
...
# --- config ---
WORKSPACE="${EDGESLAM_WORKSPACE:-/office/dev_workspace/morshed/edge-gfm-slam}"
...
echo "==> [3] Clone MASt3R-SLAM (--no-deps install; do NOT let it pull torch==2.2)"
mkdir -p "${EXT_DIR}"
if [ ! -d "${EXT_DIR}/MASt3R-SLAM" ]; then
  git clone "${MAST3R_URL}" "${EXT_DIR}/MASt3R-SLAM"
else
  echo "    MASt3R-SLAM already present — skipping clone."
fi
echo "    >>> MANUAL GATE: install curated deps yourself, THEN:"
echo "        pip install -e ${EXT_DIR}/MASt3R-SLAM --no-deps"
echo "    (script does not auto-install to avoid a blind dep pull.)"

echo "==> [4] Re-verify Blackwell survived the install"
verify_blackwell

echo "==> [5] Tooling gates — probe quant toolchain on GPU-1 (log pass/fail + versions)"
python - <<'PY'
import importlib
