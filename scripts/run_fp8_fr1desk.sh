#!/usr/bin/env bash
# Phase-2: fr1/desk with fake FP8-e4m3 (W+A) on trunk.
# Usage: bash scripts/run_fp8_fr1desk.sh [trunk|encoder]
set -eo pipefail
...
patch_load_mast3r(scope="$SCOPE", method="fake_fp8")
